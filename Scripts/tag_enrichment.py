...
to_ignore = ["loved", "tantotempotaste", "sexy"]
tags_per_topic = dict()
...
#Generate background levels
bg = dict()
sum_all = 0
for i in range(1,t_topics+1):
    for tag in tags_per_topic[i]:
        if tag not in bg:
            bg[tag] = 0
        bg[tag]+= tags_per_topic[i][tag]
        sum_all += tags_per_topic[i][tag]

for tag in bg:
    bg[tag]/= sum_all

#generate proportions from counts
sum_all = dict()
tags_per_topic_prop = copy.deepcopy(tags_per_topic)
for i in range(1,t_topics+1):
    sum_all[i] = sum(tags_per_topic[i].values())
    for tag in tags_per_topic_prop[i]:
        tags_per_topic_prop[i][tag]/=sum_all[i]


#Check enrichment for each cluster
enriched = dict()
for topic in tags_per_topic_prop:
    enriched[topic] = dict()
    for tag in tags_per_topic_prop[topic]:
        enrichment = (tags_per_topic_prop[topic][tag] - bg[tag])/tags_per_topic_prop[topic][tag]
        if enrichment > 0.7:
            enriched[topic][tag] = int(enrichment*100)  

#Generate Wordcloud
n_rows = (t_topics+1)//2
fig, ax = plt.subplots(2,n_rows, figsize = (20,7))
fig.suptitle = 'Enrichment of tags over T-Topics'
...
for i in range (1,t_topics+1):
    ...
    wordcloud = WordCloud(background_color='black',
                          colormap = "Pastel2",
                          collocations=False).generate_from_frequencies(enriched[i])                     
    if i <(n_rows + 1):
        b = 0
        a = i
    else:
    ...
    ax[b][a-1].axis("off")
fname = results_path + 'enriched_tags_wordcloud_' + str(h_topics) + "H" + str(t_topics) + "T" + dtp + ".png"
fig.savefig(fname)

print("The enriched tag-wordclouds have been saved in the results directory.")